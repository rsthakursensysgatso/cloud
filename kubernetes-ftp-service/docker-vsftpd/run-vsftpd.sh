...
mkdir -p /ftpusers
FILE="/ftpusers/virtual_users.txt"
if [ -f "$FILE" ]; then
    /usr/bin/db_load -T -t hash -f /ftpusers/virtual_users.txt /ftpusers/virtual_users.db
else
    echo -e "${FTP_USER}\n${FTP_PASS}" > /ftpusers/virtual_users.txt
    /usr/bin/db_load -T -t hash -f /ftpusers/virtual_users.txt /ftpusers/virtual_users.db
fi
...
echo "rsa_cert_file=${SSL_CERT}" >> /etc/vsftpd/vsftpd.conf
echo "rsa_private_key_file=${SSL_PRIVATE_KEY}" >>  /etc/vsftpd/vsftpd.conf
echo "ssl_enable=${SSL_ENABLE}" >>  /etc/vsftpd/vsftpd.conf
echo "ssl_ciphers=${SSL_CIPHER}" >> /etc/vsftpd/vsftpd.conf
...
echo "force_local_logins_ssl=${FORCE_LOCAL_LOGIN_SSL}" >> /etc/vsftpd/vsftpd.conf
fi
